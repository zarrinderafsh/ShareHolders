...
public class SyncPersonalPic {
	...
	//Contractor
	public SyncPersonalPic(Activity activity,String PGuid,boolean ShowDialog,boolean LoadActivityAfterExecute) {
	...
	private class AsyncCallWS extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
        	if(result == null)
        	{
	            ...
	            {
	            	//Toast.makeText(this.activity.getApplicationContext(), PersianReshape.reshape("خطا در بروزرسانی عکس پرسنلی"), Toast.LENGTH_LONG).show();
	            }
	            else if(WsResponse.toString().compareTo("Nothing") == 0)
	            {
	            	db = dbh.getWritableDatabase();
	                db.execSQL("delete from personImage");
	            	WsResponse = "Nothing";
	            }
	            else
	            {
	...
	public void InsertDataFromWsToDb(String AllRecord)
    {
        ...
        for(int i = 0 ; i < CuAllRecord.length;i++)
        {
            ...
            {
            	db = dbh.getWritableDatabase();
            	db.execSQL("insert into personImage(personimage) values('"+AllFields[0].toString()+"')");
            }
        }
