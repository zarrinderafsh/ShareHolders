...
public class ShareSalesAndBuyAdapter extends BaseAdapter {
	public ArrayList<HashMap<String, String>> list;
	Activity activity;
	InternetConnection IC;

	public ShareSalesAndBuyAdapter(Activity activity,
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(map.get("type").toString().compareTo("درخواست فروش")==0)
		{
			holder.txtrequesttype.setTextColor(Color.RED);
		}
		else
		{
			holder.txtrequesttype.setTextColor(Color.parseColor("#4cc131"));
		}
		holder.txtrequesttype.setText(map.get("nType"));
		holder.txtalldate.setText(Html.fromHtml("تاریخ : "+map.get("sDate")+" <br /><br /> "+" تعداد : "+map.get("count")+" - "+" وضعیت : "+map.get("nStatus")));
		holder.txtrequesttype.setTag(map.get("id")+"##"+map.get("type")+"##"+map.get("PGuid"));
		holder.txtalldate.setTag(map.get("id")+"##"+map.get("type")+"##"+map.get("PGuid"));
			 ...
			 Builder alertbox = new AlertDialog.Builder(activity);
		       ...
		       alertbox.setNegativeButton("بله", new DialogInterface.OnClickListener() {
		           ...
		           public void onClick(DialogInterface arg0, int arg1) {
		        	   ...
		        	   SendCancelRequestToWs(item.split("##")[1].toString(),item.split("##")[0].toString(),item.split("##")[2].toString());
		           }
		       });
	...
	private class AsyncCallWSSendCancelRequest extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... params) {
        	if(cuReqType.compareTo("درخواست فروش") == 0)
        	{
        		CallWsMethodLogin("CancelSellRequest",cuReqType,cuReqId,cuPGuid);
        	}
        	else
        	{
        		CallWsMethodLogin("CancelBuyRequest",cuReqType,cuReqId,cuPGuid);
        	}
            return null;
        }
        ...
        protected void onPostExecute(Void result) {
            if(WsResault.toLowerCase().compareTo("true")==0)
            {
				...
				SyncBuyAndSellRequests SBASR = new SyncBuyAndSellRequests(activity, cuPGuid,true,false);
				SBASR.AsyncExecute();
				//update Buy And Sell Requests
            }
            else
