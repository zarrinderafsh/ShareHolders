...
public class SyncAboutUs {
	...
	//Contractor
	public SyncAboutUs(Activity activity,String PGuid,boolean ShowDialog) {
	...
	private class AsyncCallWS extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
        	if(result == null)
        	{
	            if(WsResponse.toString().compareTo("Error Authenticate") == 0 || WsResponse.toString().compareTo("ER") == 0 )
	            {
	            	//Toast.makeText(this.activity.getApplicationContext(), PersianReshape.reshape("خطا در بروزرسانی درباره ما"), Toast.LENGTH_LONG).show();
	            }
	            else if(WsResponse.toString().compareTo("Nothing") == 0)
	            {
	            	db = dbh.getWritableDatabase();
	                db.execSQL("delete from aboutus");
	            }
	            else
	            {
	...
	public void InsertDataFromWsToDb(String AllRecord)
    {
        ...
        String[] CuAllRecord = AllRecord.split(Pattern.quote("#recordsplitter#"));
        
        String[] AllFields;        
        for(int i = 0 ; i < CuAllRecord.length;i++)
        {
            AllFields = CuAllRecord[i].split(Pattern.quote("#fieldsplitter#"));
