...
public class SyncLawyers {
	...
	//Contractor
	public SyncLawyers(Activity activity,String PGuid,boolean ShowDialog,boolean LoadActivityAfterExecute) {
	...
	private class AsyncCallWS extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
        	if(result == null)
        	{
	            ...
	            {
	            	//Toast.makeText(this.activity.getApplicationContext(), PersianReshape.reshape("خطا در بروزرسانی لیست گیرنده های پیام"), Toast.LENGTH_LONG).show();
	            }
	            else if(WsResponse.toString().compareTo("Nothing") == 0)
	            {
	            	db = dbh.getWritableDatabase();
	                db.execSQL("delete from lawyers");
	            	WsResponse="Nothing";
	            }
	            else
	            {
	...
	public void InsertDataFromWsToDb(String AllRecord)
    {
        ...
        for(int i = 0 ; i < CuAllRecord.length;i++)
        {
            ...
            {
            	db = dbh.getWritableDatabase();
            	db.execSQL("insert into lawyers(id,fullName,mobile,utype) values("+AllFields[0].toString()+",'"+AllFields[1].toString()+"','"+AllFields[2].toString()+"','"+AllFields[3].toString()+"')");
            }
        }
