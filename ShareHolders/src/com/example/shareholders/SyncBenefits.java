...
public class SyncBenefits {
	...
	//Contractor
	public SyncBenefits(Activity activity,String PGuid,boolean ShowDialog) {
	...
	private class AsyncCallWS extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
        	if(result == null)
        	{
	            ...
	            {
	            	//Toast.makeText(this.activity.getApplicationContext(), PersianReshape.reshape("خطا در بروزرسانی سود مصوب"), Toast.LENGTH_LONG).show();
	            }
	            else if(WsResponse.toString().compareTo("Error Authenticate") == 0 || WsResponse.toString().compareTo("No Benefits")==0)
	            {
	            	Toast.makeText(this.activity.getApplicationContext(), PersianReshape.reshape("سود مصوب ثبت نشده است"), Toast.LENGTH_LONG).show();
	            }
	            else
	            {
	...
	public void InsertDataFromWsToDb(String AllRecord)
    {
        ...
        db.execSQL("delete from benefits");
        ...
        for(int i = 0 ; i < CuAllRecord.length;i++)
        {
            ...
            {
            	db = dbh.getWritableDatabase();
            	db.execSQL("insert into benefits(title,cost) values('"+AllFields[1].toString()+"','"+AllFields[2].toString()+"')");
            }
        }
        db.close();
        dbh.close();
