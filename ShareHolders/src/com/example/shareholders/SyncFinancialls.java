...
public class SyncFinancialls {
	...
	//Contractor
	public SyncFinancialls(Activity activity,String PGuid,boolean ShowDialog) {
	...
	private class AsyncCallWS extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
        	if(result == null)
        	{
	            ...
	            {
	            	//Toast.makeText(this.activity.getApplicationContext(), PersianReshape.reshape("خطا در بروزرسانی اطلاعات مالی"), Toast.LENGTH_LONG).show();
	            }
	            else if(WsResponse.toString().compareTo("No Price") == 0)
	            {
	            	db = dbh.getWritableDatabase();
	                db.execSQL("delete from financials");
	            	WsResponse = "No Price";
	            }
	            else
	            {
	...
	public void InsertDataFromWsToDb(String AllRecord)
    {
        ...
        for(int i = 0 ; i < CuAllRecord.length;i++)
        {
            ...
            {
            	db = dbh.getWritableDatabase();
            	db.execSQL("insert into financials(id,description,price)"+
            	" values(1,'"+AllFields[0].toString()+"','"+AllFields[1].toString()+"')");
            }
        }
        db.close();
        dbh.close();
