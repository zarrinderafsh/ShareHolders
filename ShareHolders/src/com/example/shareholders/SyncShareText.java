...
public class SyncShareText {
	...
	//Contractor
	public SyncShareText(Activity activity,String PGuid,boolean ShowDialog,boolean LoadActivityAfterExecute) {
	...
	private class AsyncCallWS extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
        	if(result == null)
        	{
	            ...
	            {
	            	//Toast.makeText(this.activity.getApplicationContext(), PersianReshape.reshape("خطا در بروزرسانی متون درخواست خرید و فروش"), Toast.LENGTH_LONG).show();
	            }
	            else if(WsResponse.toString().compareTo("Nothing") == 0)
	            {
	            	db = dbh.getWritableDatabase();
	                db.execSQL("delete from settings where name='ShareTextBuy' or name='ShareTextSale'");
	            	WsResponse = "Nothing";
	            }
	            else
	            {
	...
	public void InsertDataFromWsToDb(String AllRecord)
    {
        ...
        for(int i = 0 ; i < CuAllRecord.length;i++)
        {
            ...
            {
            	db = dbh.getWritableDatabase();
            	if(AllFields[0].toString().compareTo("1")==0)
            	{
	            	db.execSQL("insert into settings(name,value)"+
	            	" values('ShareTextBuy','"+AllFields[1].toString()+"')");
            	}
            	else if(AllFields[0].toString().compareTo("2")==0)
            	{
	            	db.execSQL("insert into settings(name,value)"+
	            	" values('ShareTextSale','"+AllFields[1].toString()+"')");
            	}
