...
public class SyncCityes {
	...
	//Contractor
	public SyncCityes(Activity activity,String PGuid,boolean ShowDialog) {
	...
	private class AsyncCallWS extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
        	if(result == null)
        	{
	            ...
	            {
	            	//Toast.makeText(this.activity.getApplicationContext(), PersianReshape.reshape("خطا در بروزرسانی لیست شهرها"), Toast.LENGTH_LONG).show();
	            }
	            else
	            {
	...
	public void InsertDataFromWsToDb(String AllRecord)
    {
        ...
        for(int i = 0 ; i < CuAllRecord.length;i++)
        {
            ...
            {
            	db = dbh.getWritableDatabase();
            	db.execSQL("insert into cities(id,name) values("+AllFields[0].toString()+",'"+AllFields[1].toString()+"')");
            }
        }
        db.close();
        dbh.close();
