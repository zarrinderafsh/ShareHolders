...
public class Ground extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        spec.setIndicator(PersianReshape.reshape("زمین ها"),getResources().getDrawable(R.drawable.tabhistory));
        tabs.addTab(spec);
...
//        tabs.getTabWidget().getChildAt(1).setBackgroundColor(Color.RED);
        ...
        lst_data=(ListView)findViewById(R.id.groundlistView1);
		...
		try {
   		...
   		lst_dataRequest=(ListView)findViewById(R.id.groundRequestlistView1);
	...
	public void FillDataRequest()
   	{
   		...
   		Cursor cursors = dbRequest.rawQuery("select Code,RequestType,ObjectCode,GroundAparteman,Status,InsertDate from AptGroRequest where RequestType = 2 order by Code desc", null);
	...
	public void FillData()
   	{
   		...
   		Cursor cursors = db.rawQuery("select Code,Metraj,Abad,MogheeyatJoghrafi,MogheeyatMakani,Address,Sanad,TedadShoraka,Tozihat,Gheymat from ground order by Code desc", null);

   		if(cursors.getCount() > 0)
   		{
   			String Code,Metraj,Abad,MogheeyatJoghrafi,MogheeyatMakani,Address,Sanad,TedadShoraka,Tozihat,Gheymat;
   			...
   			for (int i = 0; i < cursors.getCount(); i++) {
   				...
   				Abad = cursors.getString(cursors.getColumnIndex("Abad"));
   				MogheeyatJoghrafi = cursors.getString(cursors.getColumnIndex("MogheeyatJoghrafi"));
   				MogheeyatMakani = cursors.getString(cursors.getColumnIndex("MogheeyatMakani"));
   				Address = cursors.getString(cursors.getColumnIndex("Address"));
   				Sanad = cursors.getString(cursors.getColumnIndex("Sanad"));
   				TedadShoraka = cursors.getString(cursors.getColumnIndex("TedadShoraka"));
   				Tozihat = cursors.getString(cursors.getColumnIndex("Tozihat"));
   				Gheymat = cursors.getString(cursors.getColumnIndex("Gheymat"));
   				map.put("PGuid", getIntent().getStringExtra("PGuid").toString());
   				map.put("Code", Code);
   				map.put("Metraj", Metraj);
   				map.put("Abad", Abad);
   				map.put("MogheeyatJoghrafi", MogheeyatJoghrafi);
   				map.put("MogheeyatMakani", MogheeyatMakani);
   				map.put("Address", Address);
   				map.put("Sanad", Sanad);
   				map.put("TedadShoraka", TedadShoraka);
   				map.put("Tozihat", Tozihat);
   				map.put("Gheymat", Gheymat);
   				DataLList.add(map);
   			}
   			adapter = new GroundAdapter(this, DataLList);
   			lst_data.setAdapter(adapter);
   		}
   		else
   		{
   			Toast.makeText(getApplicationContext(), PersianReshape.reshape("اطلاعات زمین ها موجود نمی باشد ، لطفا بروزرسانی کنید"), Toast.LENGTH_LONG).show();
   		}
	...
	private void ExitApplication()
	{
		//Exit All Activity And Kill Application
		 Builder alertbox = new AlertDialog.Builder(Ground.this);
